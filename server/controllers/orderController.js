...
export const createVnpayPaymentUrl = async (req, res) => {
  try {
    ...
    // MOCK MODE: khi VNPay sandbox credentials chưa có, dùng local mock endpoint.
    // Không commit production. Flag bằng VNPAY_MOCK_MODE=true trong .env.
    if (process.env.VNPAY_MOCK_MODE === "true") {
      // Ensure order có paymentGatewayTxnRef để mock callback có thể tra cứu.
      if (!order.paymentGatewayTxnRef) {
        order.paymentGatewayTxnRef = `MOCK${order._id.toString().slice(-8)}${Date.now()}`;
        order.paymentStatus = "pending";
        order.failedAt = undefined;
        await order.save();
      }
      const mockUrl = `${getBackendBaseUrl()}/api/orders/payment/mock/${order._id}`;
      logger.warn(
        { orderId: String(order._id) },
        "VNPAY_MOCK_MODE active — dùng mock payment URL",
      );
      return res.json({
        success: true,
        paymentUrl: mockUrl,
...
// MOCK endpoint — giả lập VNPay callback khi không có sandbox credentials.
// CHỈ hoạt động khi VNPAY_MOCK_MODE=true. Render trang nhỏ với 2 nút:
// "Thanh toán thành công" và "Thanh toán thất bại" để giống trải nghiệm thật.
export const handleVnpayMockPage = async (req, res) => {
  if (process.env.VNPAY_MOCK_MODE !== "true") {
    return res.status(404).send("Mock mode is not enabled");
  }

  const { orderId } = req.params;
  const order = await Order.findById(orderId).populate("courseId", "name");
  if (!order) {
    return res.status(404).send("Order not found");
  }

  const amount = Number(order.amount || 0).toLocaleString("vi-VN");
  const courseName = (order.courseId?.name || order.courseName || "Khóa học")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");

  // Trang HTML đơn giản, không cần template engine.
  res.set("Content-Type", "text/html; charset=utf-8");
  res.send(`<!doctype html>
<html lang="vi">
<head>
  <meta charset="utf-8" />
  <title>Mock VNPay — LearnLoom Dev</title>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <style>
    body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
      background: #f3f4f6; margin: 0; display: flex; align-items: center;
      justify-content: center; min-height: 100vh; color: #111827; }
    .card { background: #fff; padding: 2rem; border-radius: 12px;
      box-shadow: 0 10px 30px rgba(0,0,0,.1); max-width: 480px; width: 90%; }
    h1 { margin: 0 0 .5rem; font-size: 1.3rem; color: #dc2626; }
    ...
    .info { margin: 1rem 0; padding: 1rem; background: #f9fafb;
      border-radius: 8px; }
    ...
    .btn { flex: 1; padding: .75rem 1rem; border: 0; border-radius: 8px;
...
<body>
  <div class="card">
    <div class="notice">
      ⚠️ <strong>MOCK MODE</strong> — VNPay sandbox chưa cấu hình.
      Trang này giả lập callback thanh toán cho mục đích demo.
    </div>
    <h1>Giả lập VNPay</h1>
    <p>Chọn kết quả để tiếp tục flow:</p>
    <div class="info">
      <div><span>Khóa học</span><strong>${courseName}</strong></div>
      <div><span>Số tiền</span><strong>${amount} VND</strong></div>
      <div><span>Mã đơn</span><strong>${String(order._id).slice(-8)}</strong></div>
    </div>
    <div class="actions">
      <a class="btn btn-success"
        href="/api/orders/payment/mock/${order._id}/resolve?result=success">
        ✓ Thanh toán thành công
      </a>
      <a class="btn btn-fail"
        href="/api/orders/payment/mock/${order._id}/resolve?result=fail">
        ✗ Thanh toán thất bại
      </a>
    </div>
  </div>
...
// Xử lý kết quả mock: update order + enroll + redirect về FE /payment-redirect.
export const handleVnpayMockResolve = async (req, res) => {
  if (process.env.VNPAY_MOCK_MODE !== "true") {
    return res.status(404).send("Mock mode is not enabled");
  }

  const { orderId } = req.params;
  const result = req.query.result === "success" ? "success" : "fail";
  ...
  // Idempotent: nếu order đã success, không xử lý lại.
  const alreadyProcessed = order.paymentStatus === "success";
  if (!alreadyProcessed) {
  ...
  logger.warn(
    ...
    "Mock VNPay result processed",
  );

  const frontendBaseUrl = getFrontendBaseUrl();
  const redirectParams = new URLSearchParams({
    status: alreadyProcessed ? order.paymentStatus : result,
    message: `Mock ${result}`,
    orderId: String(order._id),
    transactionId: order.transactionId || "",
    txnRef: order.paymentGatewayTxnRef || "",
  });
